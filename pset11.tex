...
\question Prove the following using mathematical induction on $n\in\mathbb{Z}$ where $n>0$.
    ...
    \begin{solution}
      ...
      \underline{Base case}: $P(1)$:\\
        \begin{align*}
          ...
          \implies 133 &= 7m
      \end{align*}
      The above expression true when $m=19$. Therefore, $P(1)$ is true.\\
      The base case for the proof by mathematical induction holds.

      \underline{Inductive step}:\\
        \begin{align*}
          IH: P(k): 5^{2k+1}+2^{2k+1} = 7m &\implies & 5^{2k+1} &= 7m - 2^{2k+1} \\
                                           &\implies & 5^25^{2k+1} &= 5^2(7m - 2^{2k+1})\\
                                           &\implies & 5^{2(k+1)+1} &= 5^27m - 5^22^{2k+1}\\
                                           &\implies & 5^{2(k+1)+1} + 2^{2(k+1)+1} & = 5^27m - 5^22^{2k+1} + 2^{2k+3}\\
                                           & &  & = 5^27m + 2^{2k+3} - 5^22^{2k+1} \\
                                           & &  & = 5^27m + 2^{2k+1}(2^2 - 5^2) \\
                                           & &  & = 5^27m + 2^{2k+1}(4 - 25) \\
                                           & &  & = 5^27m + 2^{2k+1}(- 21) \\
                                           & &  & = 7(5^2m - 2^{2k+1}3) \\
                                           & &  & = 7m_1 \\
  ...
  \part if $A_1, A_2,\ldots,A_n$ and $B$ are sets, then
    ...
    \]
    \begin{solution}
      We have to prove $P(n): \bigcup_{i=1}^n(A_i-B)=(\bigcup_{i=1}^nA_i) -B$.\\
      We provide a proof by mathematical induction.
      \begin{proof} $P(n)$ is true for all integers greater than $0$.
